...
users.find(user => user.firstName === 'Madison' && user.lastName === 'Marshall').id;

users.find(user => user.id === comments[0].userId);

comments.find(comment => comment.text === 'Great Job!').userId;

users.find(user => user.id === comments[0].userId).firstName + ' ' + users.find(user => user.id === comments[0].userId).lastName;
